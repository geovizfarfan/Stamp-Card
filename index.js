...
if (!TOKEN || !GUILD_ID || !REWARD_ROLE_ID) {
  console.error("Missing required env vars. Check .env (DISCORD_TOKEN, GUILD_ID, REWARD_ROLE_ID)");
  process.exit(1);
}

if (!STAMP_LOG_CHANNEL_ID) console.warn("⚠️ STAMP_LOG_CHANNEL_ID not set — stamp logs will be disabled.");
if (!STAMP_COMPLETED_CHANNEL_ID) console.warn("⚠️ STAMP_COMPLETED_CHANNEL_ID not set — completed posts will be disabled.");
...
async function getCount(guildId, userId, cardId) {
  const res = await pool.query(
    "SELECT count FROM stamps WHERE guild_id=$1 AND user_id=$2 AND card_id=$3",
...
async function deleteCount(guildId, userId, cardId) {
  await pool.query(
    "DELETE FROM stamps WHERE guild_id=$1 AND user_id=$2 AND card_id=$3",
    [guildId, userId, cardId]
  );
}
...
async function getCard(guildId, userId) {
  const res = await pool.query(
    "SELECT card_id FROM user_cards WHERE guild_id=$1 AND user_id=$2",
    [guildId, userId]
  );
...
async function countCompleted(guildId, userId) {
  const res = await pool.query(
    "SELECT COUNT(*) as total FROM completed_cards WHERE guild_id=$1 AND user_id=$2",
    [guildId, userId]
  );
...
async function getHistory(guildId, userId) {
  const res = await pool.query(
    `SELECT card_id, card_number, completed_at FROM completed_cards
     WHERE guild_id=$1 AND user_id=$2 ORDER BY completed_at DESC`,
    [guildId, userId]
  );
  return res.rows;
}
...
async function renderStampCard(cardId, stampCount) {
  ...
  for (let i = 0; i < filled; i++) {
    ...
    ctx.drawImage(stamp, cx - style.w / 2 + style.dx, cy - style.h / 2 + style.dy, style.w, style.h);
  }
...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    if (sub === "leaderboard") {
      ...
      if (!rows.length) {
        return interaction.reply({ content: "📊 No stamps have been issued yet.", ephemeral: true });
      }
      let rank = 1;
      const lines = [];
    ...
    if (sub === "history") {
      ...
      if (!rows.length) {
        return interaction.reply({ content: `📭 **${user.username}** hasn't completed any stamp cards yet.`, ephemeral: true });
      }
    ...
    if (sub === "view") {
      ...
      if (!STAMP_CARDS[cardId]) {
        return interaction.reply({ content: "❌ Your saved card is invalid. Run `/stamp setcard` again.", ephemeral: true });
      }
    ...
    if (sub === "resetall") {
      ...
      const isAdmin = interaction.memberPermissions?.has(PermissionFlagsBits.Administrator);
      if (!isOwner && !isAdmin) {
        return interaction.reply({ content: "❌ Only the server owner or admins can reset the entire server.", ephemeral: true });
      }
    ...
    if (sub === "add" || sub === "remove" || sub === "reset") {
      if (!canManage(interaction)) {
        return interaction.reply({ content: "❌ You don't have permission to manage stamps.", ephemeral: true });
      }
      ...
      const cardId = savedCard || "og";
      if (!STAMP_CARDS[cardId]) {
        return interaction.reply({ content: "❌ That user has an invalid saved card. Ask them to run `/stamp setcard`.", ephemeral: true });
      }
      ...
      if (sub === "reset") {
        ...
        await logStampWithImage({
          interaction, targetUser, cardId: activeCardId,
      ...
      if (sub === "add" && current < STAMP_GOAL && next >= STAMP_GOAL) {
        ...
        await postCompletedWithImage({ interaction, targetUser, cardId, count: next, cardNumber });
        await deleteCount(guildId, targetUser.id, cardId);
        if (!targetMember.roles.cache.has(REWARD_ROLE_ID)) {
          await targetMember.roles.add(REWARD_ROLE_ID).catch(() => {});
        }
        await logStampWithImage({ interaction, targetUser, cardId, action: `🏅 Card #${cardNumber} COMPLETED & reset for next card`, count: next });
        return interaction.reply(
