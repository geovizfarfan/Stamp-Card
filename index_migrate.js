...
client.on("interactionCreate", async (interaction) => {
  ...
  try {
    ...
    if (sub === "add" || sub === "remove" || sub === "reset") {
      ...
      // Get stamp — from campaign card preference, user_cards, or staff stamp
      ...
      const stampId = overrideStamp || campCardForStamp?.stamp_id || savedCardInfo?.stamp_id || await getStaffStamp(guildId, interaction.user.id);
